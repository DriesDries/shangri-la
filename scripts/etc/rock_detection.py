...
class RockDetection():
    ...
    def rock_detection(self,src):
        ...
        # ror image
        cv2.imshow('sm_ror', self.display_result((src*0.8).astype(np.uint8),sm_ror,'fill','r'))
    ...
    def sm_rd(self,img):
        ...
        '''display results'''###########################################################
        cv2.imshow('Original Saliency Map',ori_sm)        
        cv2.imshow('d_e Saliency Map', sm)        

        cv2.imshow('sm_seed_img',self.display_result(bil_img,sm_seed_img,'fill','r'))
    ...
    def vj_rd(self,img):
        ...
        vj_ror, bil_img = rgrow.rg_vj(img,vj_seed_list)
...
class ViolaJones():
    ...
    def vj_main(self,src,sun_direction):
        ...
        kernel3,kernel5,kernel7 = map(lambda x:cv2.getGaborKernel(ksize = (x,x), sigma = 5,theta = sun_direction, lambd = x, gamma = 25/x,psi = np.pi * 1/2),[3,5,7])
...
class RegionGrowing():
    ...
    def __init__(self):
        ...
        ave_kernel = np.array([[1/9.,1/9.,1/9.],
                               [1/9.,1/9.,1/9.],
                               [1/9.,1/9.,1/9.]])
    ...
    def rg_sm(self,src,sm,seed_list):
        ...
        # 岩領域の画像用意
        rors = np.zeros_like(img).astype(np.uint8)

        # bilateralfilterによる処理
        img = cv2.bilateralFilter(img, d=10, sigmaColor=100, sigmaSpace=20)
    ...
    def growing_sm(self,src,sm,x,y):
        ...
        # print start - time.time()
            ...
            # 処理中断条件                        
            if x == 0 or x >= len(src[0])-1 or y == 0 or y >= len(src[:,0])-1: # 画像の端だったらbreak
                break
            ...
            for u,v in zip([x,x-1,x+1,x],[y-1,y,y,y+1]):
                ...
                # 条件式
                if 30 > abs(img[v,u]-img[y,x]) + 35*sm[v,u]:                
    ...
    def growing_vj(self,src,x,y):
        ...
        for i in xrange(100000):
            ...
            for u,v in zip([x,x-1,x,x+1,x],[y-1,y,y,y,y+1]):
                ...
                elif 3 > abs(img[v,u] - img[sy,sx])  or img[v,u] < 20:
