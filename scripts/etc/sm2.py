# -*- coding: utf-8 -*-
'''
    IntensitySMの実装、テスト版だから自由にいじってok
...
class SaliencyMap():
    ...
    def GetIntensityCM(self,img):
        ...
        pyr = self.GetGaussianPyramid(img) 
        pyr = np.array(pyr).astype(np.float)

        # Get 6 Difference of Gaussians
        ...
        # dog[dog>10]=255
        cv2.imshow('dog',dog)
        # cv2.imshow('dog2',dog2/np.amax(dog2))
        # cv2.imshow('smsmsm3',dog3/np.amax(dog3))


        # intensity_dog = self.GetDoG(pyr) 
